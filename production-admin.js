...
function setScope(scope){
  ...
  if($('adminBrandScope'))$('adminBrandScope').textContent='Administration';
...
function openAccessAction(kind,uid){
  ...
  if(kind==='sample-data')renderSampleDataAction();
  if(kind==='user-delete')renderUserDeleteAction();
...
function localDateString(){
  const d=new Date(); const pad=n=>String(n).padStart(2,'0'); return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}`;
}
function passwordToggleMarkup(inputId){
  return `<button class="password-visibility-toggle" data-password-toggle="${esc(inputId)}" type="button" aria-label="Show password" aria-pressed="false"><svg class="password-eye-open" aria-hidden="true" viewBox="0 0 24 24"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.7"/></svg><svg class="password-eye-closed" aria-hidden="true" viewBox="0 0 24 24"><path d="m3 3 18 18"/><path d="M10.6 6.1A10.6 10.6 0 0 1 12 6c6 0 9.5 6 9.5 6a17.7 17.7 0 0 1-3 3.7M6.4 6.5C3.8 8.2 2.5 12 2.5 12s3.5 6 9.5 6a9.8 9.8 0 0 0 3.1-.5"/><path d="M9.9 9.8a3 3 0 0 0 4.3 4.3"/></svg></button>`;
}
function renderSampleDataAction(){
  const d=state.selectedUserDetail||{}, sample=d.sampleData||{};
  $('productionActionKicker').textContent='Screenshot Sample Data'; $('productionActionTitle').textContent=sample.managed?'Refresh Rolling Sample Data':'Load Rolling Sample Data';
  $('productionActionBody').innerHTML=`<div class="production-action-note"><strong>Designed for repeatable app screenshots.</strong> The same curated sources, shopping accounts, refund accounts, and logical product orders are regenerated with order/milestone/refund dates relative to the selected anchor day. This keeps Ordered, Awaiting Delivery, Obligation Required, Waiting for Review, Refund Pending, Partially Refunded, Refund Overdue, and Archived examples current over time.</div><div class="production-warning"><strong>This replaces the personal profile’s current app data.</strong> Existing orders, sources, shopping accounts, refund accounts, and the profile snapshot are replaced. Authentication, devices, deviceTrials, subscription state, Admin Plus/App Review grants, and trial grants are left untouched.</div><div class="beta-field"><label for="productionSampleAnchor">Anchor date <span class="admin-required-inline">*</span></label><input id="productionSampleAnchor" type="date" value="${esc(localDateString())}" required/><small class="admin-email-change-help">All sample dates are stable offsets from this day. Refresh again later to roll the dataset forward.</small></div>${reasonField(sample.managed?'Example: Refresh screenshot account with current relative dates':'Example: Load consistent screenshot sample data')}<div class="production-action-footer"><button class="admin-secondary-button" data-production-cancel-action type="button">Cancel</button><button class="admin-primary-button" data-production-submit="sample-data" type="button">${sample.managed?'Refresh Sample Data':'Load Sample Data'}</button></div>`;
}
function renderUserDeleteAction(){
  const d=state.selectedUserDetail||{}, u=d.user||{}, email=d.auth?.email||u.email||'';
  $('productionActionKicker').textContent='Destructive Production Action'; $('productionActionTitle').textContent='Delete Production Account';
  const expected=`DELETE ${email}`;
  $('productionActionBody').innerHTML=`<div class="production-warning production-delete-warning"><strong>This is permanent.</strong> RebataTrack will delete the Firebase Authentication user, personal profile, orders, state snapshot, device registrations, Admin entitlement records, review records, trial overrides, and manual identity links. The anti-abuse <code>deviceTrials</code> history and Admin audit entry are retained intentionally.</div><div class="beta-field"><label for="productionDeleteConfirmation">Type <strong>${esc(expected)}</strong> <span class="admin-required-inline">*</span></label><input id="productionDeleteConfirmation" type="text" autocomplete="off" spellcheck="false" placeholder="${esc(expected)}" required/></div>${reasonField('Example: User requested permanent account deletion')}<div class="production-action-footer"><button class="admin-secondary-button" data-production-cancel-action type="button">Cancel</button><button class="production-action-button red" data-production-submit="user-delete" type="button">Delete Production Account</button></div>`;
}
function openCreateSampleAccountAction(){
  state.action={kind:'sample-create',uid:''}; $('productionActionBackdrop').hidden=false;
  $('productionActionKicker').textContent='Production Sample Account'; $('productionActionTitle').textContent='Create Screenshot Sample Account';
  $('productionActionBody').innerHTML=`<div class="production-action-note"><strong>Creates a real Production Firebase account for controlled demos/screenshots.</strong> The password is sent directly to the secure Production Admin Worker over HTTPS and is never stored in the website source, Firestore, or Admin audit log. The account is created with the same rolling sample dataset used by Refresh Sample Data.</div><div class="production-form-grid"><div class="beta-field"><label for="productionSampleName">Name <span class="admin-required-inline">*</span></label><input id="productionSampleName" type="text" value="Alex Example" required/></div><div class="beta-field"><label for="productionSampleEmail">Email <span class="admin-required-inline">*</span></label><input id="productionSampleEmail" type="email" value="demoRebataTrack@mail.com" required/></div></div><div class="beta-field"><label for="productionSamplePassword">Password <span class="admin-required-inline">*</span></label><div class="password-field-wrap"><input id="productionSamplePassword" type="password" autocomplete="new-password" placeholder="Enter the sample account password" required/>${passwordToggleMarkup('productionSamplePassword')}</div><small class="admin-email-change-help">The password is intentionally not prefilled or embedded in the public GitHub Pages source.</small></div><div class="beta-field"><label for="productionSampleCreateAnchor">Anchor date <span class="admin-required-inline">*</span></label><input id="productionSampleCreateAnchor" type="date" value="${esc(localDateString())}" required/></div><label class="production-check-row"><input id="productionSampleGrantPlus" type="checkbox" checked/><span><strong>Grant complimentary RebataTrack+</strong><small>Recommended so this screenshot account can use the Web App as well as mobile.</small></span></label>${reasonField('Example: Create maintained screenshot/demo account')}<div class="production-action-footer"><button class="admin-secondary-button" data-production-cancel-action type="button">Cancel</button><button class="admin-primary-button" data-production-submit="sample-create" type="button">Create Sample Account</button></div>`;
}
...
async function submitAction(kind,button){
  ...
  try{
    ...
    if(kind==='sample-data'){action='sample-data-refresh';payload.anchorDate=$('productionSampleAnchor')?.value||localDateString();}
    if(kind==='user-delete'){action='user-delete';payload.confirmation=String($('productionDeleteConfirmation')?.value||'').trim();}
    if(kind==='sample-create'){action='sample-user-create';payload={reason,name:String($('productionSampleName')?.value||'').trim(),email:String($('productionSampleEmail')?.value||'').trim(),password:String($('productionSamplePassword')?.value||''),anchorDate:$('productionSampleCreateAnchor')?.value||localDateString(),grantPlus:!!$('productionSampleGrantPlus')?.checked};if(!payload.name||!payload.email||!payload.password){setActionMessage('Name, email, and password are required.','error');return;}}
    ...
    const actionResult=await callProduction(action,payload);
    const reset=kind==='password-reset';
    const celebrationGrant=kind==='premium'||kind==='review'||kind==='trial';
    const sampleRefresh=kind==='sample-data', sampleCreate=kind==='sample-create', deleted=kind==='user-delete';
...
// Wait for the same authenticated Admin Portal identity already used by the existing admin portal.
